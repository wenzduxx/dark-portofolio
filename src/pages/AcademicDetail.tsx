...
import { ArrowLeft, BookOpen, MapPin, Calendar, CheckCircle2 } from 'lucide-react';
    ...
    <div className="bg-bg min-h-screen" ref={containerRef}>
      ...
      <section className="relative w-full min-h-[70vh] flex flex-col justify-end mb-24 md:mb-32 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[1200px] mx-auto px-6 md:px-12 pb-16 pt-32">
          ...
          >
            <span className="text-xs text-muted font-mono uppercase tracking-[0.3em]">{academic.school}</span>
            <div className="w-8 h-px bg-stroke" />
            <span className="text-xs text-accent font-mono uppercase tracking-[0.3em]">{academic.period}</span>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
          ...
          <div className="lg:col-span-8 space-y-24">
            ...
            <section className="fade-up-section stagger-list border-t border-stroke pt-24">
              <div className="flex items-center gap-3 mb-10">
                 ...
                 <h2 className="text-xs text-muted uppercase tracking-[0.3em]">Activities & Involvement</h2>
              </div>
              <ul className="space-y-6">
                {academic.activities.map((act, idx) => (
                  <li key={idx} className="flex gap-4 items-start stagger-item">
                    <CheckCircle2 className="w-5 h-5 text-accent shrink-0 mt-1" />
                    <span className="text-lg text-text-primary/90">{act}</span>
                  </li>
                ))}
              </ul>
            </section>
            ...
            {/* Image Gallery */}
            {academic.gallery && academic.gallery.length > 0 && (
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-32 space-y-12">
              ...
              {/* Metrics block */}
              {academic.metrics && academic.metrics.length > 0 && (
                <div className="fade-up-section space-y-4">
                  {academic.metrics.map((metric, idx) => (
