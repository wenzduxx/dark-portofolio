// Interactive pixel-canvas hover animation.
// A self-contained custom element (`<pixel-canvas>`) that paints shimmering
// pixels which grow in on parent hover/focus and shrink out on leave/blur.
// Adapted for Vite + React 19 (no "use client" directive needed).

import * as React from "react"

// First, the individual Pixel.
class Pixel {
  width: number
  height: number
...
// Then the web component that hosts the canvas.
class PixelCanvasElement extends HTMLElement {
  ...
  // Bound handlers so add/removeEventListener reference the same functions.
...
// Make TypeScript aware of the <pixel-canvas> intrinsic element (React 19 JSX).
...
const PixelCanvas = React.forwardRef<HTMLElement, PixelCanvasProps>(
  ({ gap, speed, colors, variant, noFocus, style, ...props }, ref) => {
    React.useEffect(() => {
      // Register the custom element once, on first mount.
